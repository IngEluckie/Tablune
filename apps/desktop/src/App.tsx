...
export default function App() {
  ...
  useEffect(() => {
    const handleShortcuts = (event: globalThis.KeyboardEvent) => {
      ...
      if (key === "t") {
        ...
        void command(documentId).then(updateDocument).catch((reason) => {
  ...
  }, [activateDocument, busy, createNew, macroOpen, requestCloseDocument, unsavedRequest, updateDocument]);
  ...
  const changeView = async (nextView: ViewState) => {
    ...
    try {
      ...
      updateTabUi(documentId, (current) => ({ ...current, view: nextView }));
  ...
  return (
    <main className="app-shell" data-theme={theme}>
      <RibbonHeader
        ...
        onUndo={() => void undoSession(summary.documentId).then(updateDocument).catch((reason) => setError(String(reason)))}
        onRedo={() => void redoSession(summary.documentId).then(updateDocument).catch((reason) => setError(String(reason)))}
        ...
        onHeaderChange={(enabled) => void setSessionHeader(summary.documentId, enabled).then(updateDocument).catch((reason) => setError(String(reason)))}
      ...
      {macroOpen && (
        <PythonMacroDialog
          ...
          onApplied={(nextSummary) => {
            ...
            updateTabUi(nextSummary.documentId, (current) => ({ ...current, view: EMPTY_VIEW }));
          }}
